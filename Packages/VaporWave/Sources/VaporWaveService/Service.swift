...
public class Service {
  ...
  var server : Server!
  convenience init (service : String) async throws {
    self.init()
    let listener = try XPCListener(
      service: service,
      incomingSessionHandler: self.incomingSessionHandler
    )
    async let server = try await Entrypoint.start()
  ...
  @discardableResult
  public static func launch (service : String) async throws -> Service {
    let service = try await Service(service: service)
    defer {
      dispatchMain()
    }
    return service
